package com.apexsions.crates.crate.cost.entry.impl;
...
import com.apexsions.crates.crate.cost.entry.AbstractCostEntry;
...
import com.apexsions.crates.dialog.cost.CurrencyCostOptionsDialog;
...
public class EcoCostEntry extends AbstractCostEntry<EcoCostType> {
    ...
    public NightItem getEditorIcon() {
        if (ApexsionsEconomyProvider.isAvailable()) {
            com.apexsions.economy.currency.Currency apexCurr = ApexsionsEconomyProvider.get().getCurrency(this.currencyId);
            Material iconMat = (apexCurr != null && apexCurr.getIcon() != null) 
                    ? apexCurr.getIcon() 
                    : (this.currencyId.equalsIgnoreCase("diamond") ? Material.DIAMOND : Material.EMERALD);
            String currName = apexCurr != null ? apexCurr.getDisplayName() : this.currencyId;
            return NightItem.fromType(iconMat)
                .localized(EcoCostType.LOCALE_EDIT_BUTTON)
                .replacement(replacer -> replacer
                    .replace(Placeholders.GENERIC_ID, () -> CoreLang.goodEntry(this.currencyId))
                    ...
                    .replace(Placeholders.GENERIC_NAME, () -> currName)
                )
                .hideAllComponents();
        }
    ...
    public String format() {
        if (ApexsionsEconomyProvider.isAvailable()) {
            return ApexsionsEconomyProvider.get().format(this.amount, this.currencyId);
        }
    ...
    public boolean isValid() {
        ...
        if (ApexsionsEconomyProvider.isAvailable()) {
            return ApexsionsEconomyProvider.get().getCurrency(this.currencyId) != null
                    || this.currencyId.equalsIgnoreCase("rupiah")
                    || this.currencyId.equalsIgnoreCase("diamond");
        }
        return this.currency().isPresent();
    }
    ...
    public int countPossibleOpenings(@NotNull Player player) {
        ...
        if (ApexsionsEconomyProvider.isAvailable()) {
            double bal = ApexsionsEconomyProvider.get().getBalance(player.getUniqueId(), this.currencyId);
            return (int) Math.floor(bal / this.amount);
        }
    ...
    public boolean hasEnough(@NotNull Player player) {
        if (ApexsionsEconomyProvider.isAvailable()) {
            return ApexsionsEconomyProvider.get().has(player.getUniqueId(), this.currencyId, this.amount);
        }
    ...
    public void take(@NotNull Player player) {
        if (ApexsionsEconomyProvider.isAvailable()) {
            ApexsionsEconomyProvider.get().withdraw(player.getUniqueId(), this.currencyId, this.amount);
            return;
        }
    ...
    public void refund(@NotNull Player player) {
        if (ApexsionsEconomyProvider.isAvailable()) {
            ApexsionsEconomyProvider.get().deposit(player.getUniqueId(), this.currencyId, this.amount);
            return;
        }
