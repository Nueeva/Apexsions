...
import com.apexsions.crates.crate.cost.entry.impl.EcoCostEntry;
...
public class CurrencyCostOptionsDialog extends Dialog<EcoCostEntry> {
    ...
    public WrappedDialog create(@NotNull Player player, @NotNull EcoCostEntry entry) {
        return Dialogs.create(builder -> {
            builder.base(DialogBases.builder(TITLE)
                ...
                .inputs(
                    DialogInputs.singleOption(INPUT_ID, LABEL_CURRENCY, getCurrencyOptions(entry)).build(),
    ...
    private java.util.List<WrappedSingleOptionEntry> getCurrencyOptions(@NotNull EcoCostEntry entry) {
        java.util.List<WrappedSingleOptionEntry> list = new java.util.ArrayList<>();
        if (com.apexsions.economy.api.ApexsionsEconomyProvider.isAvailable()) {
            for (com.apexsions.economy.currency.Currency c : com.apexsions.economy.api.ApexsionsEconomyProvider.get().getCurrencies()) {
                list.add(new WrappedSingleOptionEntry(c.getId(), c.getDisplayName(), entry.getCurrencyId().equalsIgnoreCase(c.getId())));
            }
        }
        if (list.isEmpty()) {
            for (su.nightexpress.nightcore.bridge.currency.Currency c : EconomyBridge.getCurrencies()) {
                list.add(new WrappedSingleOptionEntry(c.getInternalId(), c.getName(), entry.getCurrencyId().equalsIgnoreCase(c.getInternalId())));
            }
        }
        if (list.isEmpty()) {
            list.add(new WrappedSingleOptionEntry("rupiah", "Rupiah", entry.getCurrencyId().equalsIgnoreCase("rupiah")));
            list.add(new WrappedSingleOptionEntry("diamond", "Diamond", entry.getCurrencyId().equalsIgnoreCase("diamond")));
        }
        return list;
    }
}
