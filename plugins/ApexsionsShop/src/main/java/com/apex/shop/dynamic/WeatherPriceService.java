        ...
        switch (weather) {
            case CLEAR -> {
                if (category == ShopCategory.FARMING) {
                    return plugin.getConfig().getDouble("dynamic-pricing.weather.clear.farming-sell-multiplier", 1.20);
                }
                return plugin.getConfig().getDouble("dynamic-pricing.weather.clear.mob-sell-multiplier", 1.00);
            }
            case RAIN -> {
                if (category == ShopCategory.FARMING) {
                    return plugin.getConfig().getDouble("dynamic-pricing.weather.rain.farming-sell-multiplier", 0.95);
                }
                if (category == ShopCategory.MOB_DROPS) {
                    return plugin.getConfig().getDouble("dynamic-pricing.weather.rain.mob-sell-multiplier", 1.10);
                }
                return 1.00;
            }
            case THUNDER -> {
                if (category == ShopCategory.FARMING) {
                    return plugin.getConfig().getDouble("dynamic-pricing.weather.thunder.farming-sell-multiplier", 0.90);
                }
                if (category == ShopCategory.MOB_DROPS) {
                    return plugin.getConfig().getDouble("dynamic-pricing.weather.thunder.mob-sell-multiplier", 1.25);
                }
                return 1.00;
            }
            default -> {
