package com.apex.battlepass.shop.gui;
...
public class WeeklyShopMenu extends Gui {

    public WeeklyShopMenu(ApexsionsBattlepass plugin, Player player, Gui parent) {
        super(plugin, player, "&8[ &e&lWEEKLY BATTLE SHOP &8]", 45, parent);
    }
    ...
    public void initialize() {
        ...
        setButton(2, new GuiButton(new ItemBuilder(Material.GOLD_BLOCK)
                .name("&e&l[✔] WEEKLY SHOP")
                .lore(List.of("&7Menu belanja mingguan.", "&a(Kategori Sedang Dibuka)"))
                .build()));

        setButton(3, new GuiButton(new ItemBuilder(Material.DIAMOND)
                .name("&d&lMONTHLY SHOP")
                .lore(List.of("&7Menu belanja bulanan langka.", " ", "&eKlik untuk beralih ke Monthly Shop >"))
                .build(), event -> {
            new MonthlyShopMenu(plugin, player, parent).open();
        }));
        ...
        setButton(4, new GuiButton(new ItemBuilder(Material.SPYGLASS)
                ...
                .build(), event -> {
            new RarityChancesMenu(plugin, player, ShopCategory.WEEKLY, this).open();
        }));
        ...
        try {
            rupiahBal = com.apex.economy.api.ApexsionsEconomyAPI.getBalance(player.getUniqueId(), "rupiah");
        ...
        int refreshCost = plugin.getShopRefreshService().calculateRefreshCost(player, ShopCategory.WEEKLY);
        setButton(8, new GuiButton(new ItemBuilder(Material.EXPERIENCE_BOTTLE)
                .name("&e&l[🔄] REFRESH WEEKLY SHOP")
                .lore(List.of(
                        "&7Acak ulang item Weekly Shop minggu ini!",
                        " ",
                ...
                .build(), event -> {
            new ShopRefreshConfirmMenu(plugin, player, ShopCategory.WEEKLY, this).open();
        }));

        // 5. Shop Items (Exactly 10 Centered Slots)
        Collection<ShopItem> items = plugin.getShopManager().getDisplayItems(player, ShopCategory.WEEKLY);
        ...
        for (ShopItem item : items) {
            ...
            setButton(itemSlots[idx++], new GuiButton(displayItem, event -> {
                if (plugin.getShopManager().purchaseItem(player, ShopCategory.WEEKLY, item.getId())) {
