...
public class DailyShopMenu extends Gui {

    public DailyShopMenu(ApexsionsBattlepass plugin, Player player, Gui parent) {
        super(plugin, player, "&8[ &a&lDAILY BATTLE SHOP &8]", 45, parent);
    }
    ...
    public void initialize() {
        ...
        setButton(1, new GuiButton(new ItemBuilder(Material.EMERALD_BLOCK)
                .name("&a&l[✔] DAILY SHOP")
                .lore(List.of("&7Menu belanja harian.", "&a(Kategori Sedang Dibuka)"))
                .build()));
        ...
        setButton(4, new GuiButton(new ItemBuilder(Material.SPYGLASS)
                ...
                .build(), event -> {
            new RarityChancesMenu(plugin, player, ShopCategory.DAILY, this).open();
        }));
        ...
        int refreshCost = plugin.getShopRefreshService().calculateRefreshCost(player, ShopCategory.DAILY);
        setButton(8, new GuiButton(new ItemBuilder(Material.EXPERIENCE_BOTTLE)
                .name("&e&l[🔄] REFRESH DAILY SHOP")
                .lore(List.of(
                        "&7Acak ulang item Daily Shop hari ini!",
                        " ",
                ...
                .build(), event -> {
            new ShopRefreshConfirmMenu(plugin, player, ShopCategory.DAILY, this).open();
        }));

        // 5. Shop Items (Exactly 10 Centered Slots)
        Collection<ShopItem> items = plugin.getShopManager().getDisplayItems(player, ShopCategory.DAILY);
        ...
        for (ShopItem item : items) {
            ...
            setButton(itemSlots[idx++], new GuiButton(displayItem, event -> {
                if (plugin.getShopManager().purchaseItem(player, ShopCategory.DAILY, item.getId())) {
